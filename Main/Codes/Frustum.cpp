...
void C_Frustum::InitFrustum(void)
{
	// 여덟개의 점은 투영공간상의 점이다.
	// -z
...
void C_Frustum::ConvertFrustum(VTXTEX* pVtxTex, const D3DXMATRIX* pMatrix)
{
...
int C_Frustum::Culling(VTXTEX* pVtxTex, INDEX* pIndex)
{
	DWORD			dwIndex[4];
	bool			bIn[4];
	int				iTriCnt = 0;

	for (int i = 0; i < (129 - 1); ++i)
	{
		for (int j = 0; j < 129 - 1; ++j)
		{
			int			iIndex = i * 129 + j;

			dwIndex[0] = iIndex + 129;
			dwIndex[1] = iIndex + 129 + 1;
			dwIndex[2] = iIndex + 1;
			dwIndex[3] = iIndex;
...
bool C_Frustum::VertexInFrustum(const D3DXVECTOR3* pPoint)
{
	for (int i = 0; i < 6; ++i)
	{
...
bool C_Frustum::SphereInFrustum(float fRadius, const D3DXVECTOR3* pPoint)
{
	float		fDistance;
